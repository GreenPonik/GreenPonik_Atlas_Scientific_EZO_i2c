#! /usr/bin/python3
"""
@package Class to communicate with Atlas Scientific EZO sensors in I2C mode.
#######################################################################
#######################################################################
#######################################################################
...
class _AtlasEZOI2c:
    ...
    def __init__(self, bus=DEFAULT_BUS, addr=None, moduletype=""):
        ...
        if None is addr or (
            ...
            raise Exception(
                ...
                AtlasI2c.ADDR_EZO_HEXA \
                and AtlasI2c.ADDR_EZO_DECIMAL"
            )
    ...
    def print_all_registers_values(self):
        if "EC" == self._module:
            registers = self.EZO_EC_REGISTERS
        elif "PH" == self._module:
            registers = self.EZO_PH_REGISTERS
